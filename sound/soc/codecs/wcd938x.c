					 ...
					 int num_btn, bool is_micbias)
{
	...
	for (i = 0; i < num_btn; i++) {
		...
		dev_warn(component->dev, "%s: btn_high[%d]: %d, vth: %d\n",
						...
						int32_t *zdet)
{
	...
	dev_warn(component->dev, "%s: d1=%d, c1=%d, x1=0x%x, z_val=%d (milliohm)\n",
				 ...
				 int32_t *zl, int32_t *zr, s16 *d1_a)
{
	...
	dev_warn(component->dev, "%s: ramp for HPH_L, noff = %d\n",
...
z_right:
	...
	dev_warn(component->dev, "%s: ramp for HPH_R, noff = %d\n",
		__func__, zdet_param->noff);
	wcd938x_mbhc_get_result_params(component, d1_a, zdet_param->noff, &zdet);
	regmap_update_bits(wcd938x->regmap,
...
left_ch_impedance:
	...
	dev_warn(component->dev, "%s: impedance on HPH_L = %d(ohms)\n",
		__func__, *zl);
...
right_ch_impedance:
	...
	dev_warn(component->dev, "%s: impedance on HPH_R = %d(ohms)\n",
		__func__, *zr);
	...
	if ((z_diff1 * (*zl + z1Ls)) > (z_diff2 * (z1Ls + zMono))) {
		dev_warn(component->dev, "%s: stereo plug type detected\n",
			__func__);
		wcd_mbhc_set_hph_type(wcd938x->wcd_mbhc, WCD_MBHC_HPH_STEREO);
	} else {
		dev_warn(component->dev, "%s: MONO plug type detected\n",
			__func__);
		wcd_mbhc_set_hph_type(wcd938x->wcd_mbhc, WCD_MBHC_HPH_MONO);
	}
...
static void wcd938x_mbhc_moisture_config(struct snd_soc_component *component)
{
	...
	if (!wcd938x->mbhc_cfg.hphl_swh) {
		dev_warn(component->dev, "%s: disable moisture detection for NC\n",
			__func__);
		snd_soc_component_write_field(component, WCD938X_MBHC_NEW_CTL_2,
				    WCD938X_M_RTH_CTL_MASK, R_OFF);
		return;
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_warn(component->dev, "%s: zl=%u(ohms), zr=%u(ohms)\n", __func__, zl, zr);
