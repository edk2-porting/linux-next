...
static void gamepad_input_handler(struct gamepad_device *dev, struct gamepad_data *data)
{
    ...
    input_report_abs(indev, ABS_X, 0xaa0 - (data->data[6] | (data->data[7] << 8)));
    input_report_abs(indev, ABS_Y, 0xaa0 - (data->data[8] | (data->data[9] << 8)));
    input_report_abs(indev, ABS_RX, 0xaa0 - (data->data[10] | (data->data[11] << 8)));
    input_report_abs(indev, ABS_RY, 0xaa0 - (data->data[12] | (data->data[13] << 8)));
...
static int gamepad_mcu_uart_probe(struct serdev_device *serdev)
{
    ...
    for (int i = ABS_X; i <= ABS_RZ; i++)
        input_set_abs_params(gamepad_dev->dev_input, i, 0x50, 0xa50, 0, 0);
